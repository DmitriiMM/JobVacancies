...
protocol VacancyLoadProtocol {
    ...
    func getVacancyDetail(for id: String, onCompletion: @escaping (Result<VacancyDetail, Error>) -> Void)
}
...
class VacancyLoadManager: VacancyLoadProtocol {
    ...
    func getVacancyDetail(for id: String, onCompletion: @escaping (Result<VacancyDetail, Error>) -> Void) {
        DispatchQueue.main.async {
            ...
            self.client.send(request: request, type: VacancyDetail.self, onResponse: onCompletion)
        }
    }
}
